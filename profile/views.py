...
# queries = QueryString.objects.filter(qsquerystring__pk__in=qsts_pks)
    ...
    context = {'filters': filters, 'query': query,
...
def activitAnswers(request, user_id, username):
    ...
    countAnswers = Answer.objects.filter(
        answer_owner=profileData.user).exclude(is_deleted=True).count()

    # Most Votes Answer - Tab One
    most_vote_answers = Answer.objects.filter(answer_owner=user_id).annotate(
        countThem=Count('a_vote_ups')).order_by('-countThem')
    ...
    # Most Votes Answer - Tab One
    recent_edited_answers = Answer.objects.filter(
        answer_owner=user_id).order_by('-a_edited_time')
    ...
    try:
        ...
        recentEdited_answers = recentAnswerPaginator.page(
            recentAnswerPaginator.num_pages)
    ...
    try:
        ...
        newest_answers = newest_answer_paginator.page(
            newest_answer_paginator.num_pages)
    ...
    countDownVotedPosts_Q = QDownvote.objects.filter(
        downvote_by_q=user_id).count()
    ...
    getVotesCast_Final = countVotedPosts_Q + countDownVotedPosts_Q + \
        countVotedPosts_A + countDownVotedPosts_A

    # People Reached
    getAllTheViewsOfAllTheQuestion = Question.objects.filter(post_owner=profileData.user).annotate(
        total_views=Count('viewers')).aggregate(Sum('total_views'))

    reputation_graph = Reputation.objects.filter(awarded_to=user_id)[:15]

    # ----------Next Badge---------- START----------
    countComments = CommentQ.objects.filter(
        commented_by=profileData.user).count()
    ...
    getVotedOnQ = Question.objects.filter(
        qupvote__upvote_by_q=profileData.user).count()
    getVotedOnQ_Down = Question.objects.filter(
        qdownvote__downvote_by_q=profileData.user).count()
    getVotedOn = Answer.objects.filter(a_vote_ups=profileData.user).count()
    getVotedOn_Down = Answer.objects.filter(
        a_vote_downs=profileData.user).count()
    ...
    getQ_DownVotes_in_24_Hours = QDownvote.objects.filter(
        downvote_by_q=profileData.user, date__gt=last_24_hours).count()
    getA_Votes_in_24_Hours = Answer.objects.filter(
        a_vote_ups=profileData.user, date__gt=last_24_hours).count()
    getA_DownVotes_in_24_Hours = Answer.objects.filter(
        a_vote_downs=profileData.user, date__gt=last_24_hours).count()
    totalVotes = getQ_Votes_in_24_Hours + getQ_DownVotes_in_24_Hours + \
    ...
    newestBadge = TagBadge.objects.filter(
        awarded_to_user=profileData.user).last()
...
def questionsActivity(request, user_id, username):
    ...
    countQuestions = Question.objects.filter(
        post_owner=profileData.user).exclude(is_deleted=True).count()

    question_most_votes = Question.objects.filter(post_owner=user_id).exclude(
        is_deleted=True).annotate(countThem=Count('qupvote')).order_by('-countThem')

    question_recent_activity = Question.objects.filter(
        post_owner=user_id).exclude(is_deleted=True).order_by('-active_date')

    question_newest = Question.objects.filter(
        post_owner=user_id).exclude(is_deleted=True).order_by('-date')

    question_most_views = Question.objects.filter(post_owner=user_id).exclude(
        is_deleted=True).annotate(countTheViews=Count('viewers')).order_by('-viewers')
...
def tagsActivity(request, user_id, username):
    profileData = get_object_or_404(Profile, pk=user_id)
    tags = Tag.objects.filter(question__answer__answer_owner=user_id).annotate(
        answeredOn=Count('taggit_taggeditem_items'))

    totalBookmarks = BookmarkQuestion.objects.filter(
        bookmarked_by=user_id).count()
...
def bountiesActivity(request, user_id, username):
    ...
    active_bounties = Bounty.objects.filter(
        by_user=user_id, date__gt=LAST_SEVEN_DAYS)
...
def reputationActivity(request, user_id, username):
    ...
    get_rep_data = Reputation.objects.filter(
        awarded_to=user_id).order_by('-date_earned')
...
def flag_summary(request, user_id, username):
    ...
    voted_comment_flags = list(
        FlagComment.objects.filter(comment_flagged_by=user_id))
    ...
    results = sorted(chain(voted_flags, voted_comment_flags),
...
def allActionsActivity(request, user_id, username):
    profileData = get_object_or_404(Profile, pk=user_id)
    query_1 = list(Question.objects.filter(
        post_owner=user_id).order_by('date'))
    query_2 = list(Answer.objects.filter(
        answer_owner=user_id).order_by('date'))
    query_3 = list(CloseQuestionVotes.objects.filter(
        user=user_id).order_by('date'))
    query_4 = list(Bounty.objects.filter(by_user=user_id).select_related(
        'question_bounty').order_by('date'))
    query_5_comment = list(CommentQ.objects.filter(
        commented_by=user_id).order_by('date'))
    query_6_tag = list(TagBadge.objects.filter(
        awarded_to_user=user_id).order_by('date'))
    query_7_reviewing = list(ReviewCloseVotes.objects.filter(
        reviewed_by=user_id).order_by('date'))
    ...
    results = sorted(chain(query_1, query_2, query_3, query_4,
                           query_5_comment, query_6_tag), key=order, reverse=True)
    page = request.GET.get('page', 1)
    ...
    getAnsweredAnswers = Answer.objects.filter(
        answer_owner=user_id).order_by('date')
...
def Votes_castActivity(request, user_id, username):
    ...
    allPosts = sorted(chain(query_1, query_2, query_3,
                            query_4, query_5), key=order, reverse=True)
...
def activityPageTabProfile(request, user_id, username):
    ...
    goldBadges = TagBadge.objects.filter(
        awarded_to_user=profileData.user, badge_type="GOLD")[:3]
    silverBadges = TagBadge.objects.filter(
        awarded_to_user=profileData.user, badge_type="SILVER")[:3]
    bronzeBadges = TagBadge.objects.filter(
        awarded_to_user=profileData.user, badge_type="BRONZE")[:3]

    if request.user.is_authenticated:
        topTags = Tag.objects.filter(question__post_owner=request.user).annotate(
            countTheVotes=Count('question__qupvote')).filter(countTheVotes__gte=2)
    else:
        topTags = ''

    questions = Question.objects.filter(
        post_owner=profileData.user).exclude(is_deleted=True)
    answers = Answer.objects.filter(
        answer_owner=profileData.user).exclude(is_deleted=True)

    getMixTopVotes_Q = list(Question.objects.filter(post_owner=profileData.user).exclude(
        is_deleted=True).annotate(getMostVotes=Count('qupvote')).order_by('getMostVotes'))
    getMisTopVotes_A = list(Answer.objects.filter(answer_owner=profileData.user).exclude(
        is_deleted=True).annotate(getMostVotes=Count('a_vote_ups')).order_by('getMostVotes'))
    ...
    results = sorted(chain(getMixTopVotes_Q, getMisTopVotes_A),
                     key=order_by, reverse=True)
...
def bookmarksActivity(request, user_id, username):
    ...
    mostVotes_bookmarks = BookmarkQuestion.objects.filter(bookmarked_by=user_id).annotate(
        countVotes=Count('bookmarked_question__qupvote')).order_by('-countVotes')

    activity_bookmarks = BookmarkQuestion.objects.filter(
        bookmarked_by=user_id).order_by('-bookmarked_question__q_edited_time')

    newest_bookmarks = BookmarkQuestion.objects.filter(
        bookmarked_by=user_id).order_by('-date')

    views_bookmarks = BookmarkQuestion.objects.filter(bookmarked_by=user_id).annotate(
        countViews=Count('bookmarked_question__viewers')).order_by('-countViews')
...
# Question DIVs - START
    questionsCount = Question.objects.filter(
        post_owner=profileData.user).exclude(is_deleted=True).count()
    question_most_votes = Question.objects.filter(post_owner=profileData.user).annotate(
        countThem=Count('qupvote')).order_by('-countThem')[:5]
    question_recent_activity = Question.objects.filter(
        post_owner=profileData.user).order_by('-active_date')[:5]
    question_newest = Question.objects.filter(
        post_owner=profileData.user).order_by('-date')[:5]
    question_most_views = Question.objects.filter(post_owner=profileData.user).annotate(
        countTheViews=Count('viewers')).order_by('-viewers')[:5]
# Question DIVs - END
...
# Answer DIVs - START
    ...
    answersThr_Activity = Answer.objects.filter(
        answer_owner=profileData.user).order_by('-active_time')[:5]
    answersNewest = Answer.objects.filter(
        answer_owner=profileData.user).order_by('-date')[:5]
# Answer DIVs - END


# Bookmark DIVs - START
    bookmarks_newest = BookmarkQuestion.objects.filter(
        bookmarked_by=profileData.user).order_by('date')
    bookmarks_votes = BookmarkQuestion.objects.filter(bookmarked_by=profileData.user).annotate(
        countVotes=Count('bookmarked_question__qupvote')).order_by('-countVotes')
    bookmarks_activity = BookmarkQuestion.objects.filter(
        bookmarked_by=profileData.user).order_by('-bookmarked_question__active_date')
    bookmarks_views = BookmarkQuestion.objects.filter(bookmarked_by=profileData.user).annotate(
        countVotes=Count('bookmarked_question__viewers')).order_by('-countVotes')
...
# Bookmark DIVs - END
    ...
    tags = Tag.objects.filter(question__answer__answer_owner=profileData.user).annotate(
        answeredOn=Count('taggit_taggeditem_items'))[:5]

    badges = TagBadge.objects.filter(
        awarded_to_user=profileData.user).order_by('date')[:5]

    bounties = Bounty.objects.filter(
        by_user=profileData.user).order_by('date').distinct()


# Votes cast - START
    Up_votesCast_Q = QUpvote.objects.filter(
        upvote_by_q=profileData.user).count()
    Down_votesCast_Q = QDownvote.objects.filter(
        downvote_by_q=profileData.user).count()

    Up_votesCast_A = Answer.objects.filter(a_vote_ups=profileData.user).count()
    Down_votesCast_A = Answer.objects.filter(
        a_vote_downs=profileData.user).count()
...
# Votes cast - END
    ...
    getAlltheReputation = Reputation.objects.filter(
        awarded_to=profileData.user).aggregate(Sum('answer_rep_C'), Sum('question_rep_C'))
...
# Transfer this Variable into config.py
    ...
    # Total Votes Cast
    countVotedPosts_Q = QUpvote.objects.filter(
        upvote_by_q=profileData.user).count()
    countDownVotedPosts_Q = QDownvote.objects.filter(
        downvote_by_q=profileData.user).count()

    countVotedPosts_A = Answer.objects.filter(
        a_vote_ups=profileData.user).count()
    countDownVotedPosts_A = Answer.objects.filter(
        a_vote_downs=profileData.user).count()

    getVotesCast_Final = countVotedPosts_Q + countDownVotedPosts_Q + \
        countVotedPosts_A + countDownVotedPosts_A
    reputation_graph = Reputation.objects.filter(
        awarded_to=profileData.user)[:15]
...
def tagPage(request, user_id, tagbadge_id):
    ...
    allTagsGold = TagBadge.objects.filter(
        awarded_to_user=user).values_list('tag_name', flat=True)
    ...
    otherWithSame_Badge = TagBadge.objects.filter(tag_name=tag.tag_name).order_by(
        '-date').exclude(awarded_to_user__isnull=True)

    context = {'tagName': tagName, 'tag': tag, 'it_it_earned_by': it_it_earned_by,
               'user_1': user, 'allTags': allTags, 'otherWithSame_Badge': otherWithSame_Badge, }
    return render(request, 'tagbadge/user_profile_tags.html', context)


def otherWithSame_Badge(request, tag):
    # print(tag.tag_name)
    objQ = TagBadge.objects.filter(tag_name=tag).order_by(
        '-date').exclude(awarded_to_user__isnull=True)
    ...
    if request.method == 'POST':
        ...
        if Edit_profile_form.is_valid():
            ...
            if request.user.profile.about_me != '':
                print("Not Blank")
                awardBadge = TagBadge.objects.get_or_create(
                    awarded_to_user=request.user, badge_type="Bronze", tag_name="Autobiographer", bade_position="BADGE")
                sendNotification = PrivRepNotification.objects.get_or_create(
...
# Transfer this Variable into config.py
    ...
    context = {'Edit_profile_form': Edit_profile_form,
               'profileData': profileData, 'online_user_activity': online_user_activity, }
        ...
        if editProfile.is_valid():
            ...
            if request.user.profile.about_me != '':
                awardBadge = TagBadge.objects.get_or_create(
                    awarded_to_user=request.user, badge_type="Bronze", tag_name="Autobiographer", bade_position="BADGE")
                sendNotification = PrivRepNotification.objects.get_or_create(
                    for_user=request.user, url="#", type_of_PrivNotify="BADGE_EARNED", for_if="Autobiographer")
...
def userProfileJonPrefrences_Settings(request, user_id):
    ...
    context = {'online_user_activity': online_user_activity,
               'profileData': profileData, 'editProfile_Job': editProfile_Job}
...
#             message = f"{cd['review']} "
#             send_mail(subject,message,'space@gmail.com',
